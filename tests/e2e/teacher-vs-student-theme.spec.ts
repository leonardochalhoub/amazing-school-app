...
test("teacher and student dashboards use distinct theme scopes", async ({ page }) => {
  ...
  const teacherScopeCount = await page.locator(".theme-teacher").count();
  expect(teacherScopeCount).toBeGreaterThan(0);
  ...
  const studentScopeCount = await page.locator(".theme-student").count();
  expect(studentScopeCount).toBeGreaterThan(0);
});
